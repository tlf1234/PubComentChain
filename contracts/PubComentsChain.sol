...
library Data {
    ...
    struct Participant {  //这个要看是否需要。

      // 玩家推荐收益
      uint ParticipantIncome;
    ...
    struct participantActivityData{
      ...
      uint evaScore;  //影评数据   可改为U8
      uint forecastBoxOffice;  //票房预测
    ...
    struct EvaActivity {     //评定活动相关的数据
      ...
      //可能要考虑用浮点类型
      uint totalScore;//所有影评总分 
      // uint averageScore; //影评平均分
      uint totalForecastBoxOffice; //总的票房预测，单位为千万，
      ...
      mapping(address => uint) partticipantRank;  //这个数据也许可以不用
...
// 进行纯转账时，会触发回退函数。当合约收到以太但没有数据，回退函数就会执行，这是回退函数需要加上payable。
  ...
  /**
    ...
    * @param filmId 电影Id
  */
  function sponsorFight(string memory filmId) external payable{      //注意bonus需要统一一下到底是用什么币或者多种币
    //禁止合约调用   
    require(isContract(msg.sender) == false, "Not a normal user");
    //活动时间。
     // 如果拍卖已结束，撤销函数的调用。
    ...
    require(msg.value>FilmEvaActivity[filmId].bonus,"your bonus is less");
  ...
  function participateActivityWithReveal(string memory filmId,uint score,uint forecastBoxOffice) external {
    //检验活动是否在进行
    require(FilmEvaActivity[filmId].activityState==2,"activity is not in reveal state");
    require(isContract(msg.sender) == false, "Not a normal user");  //这个条件可以用一个moditify
    //注意这里还要实现加密加密校验!!!!!!!!!
  ...
  uint calcuateFactor=100;  //浮点计算放大因子，也就是说只保留两位小数。最终在最终值中除以100即可。
  //票房折合函数，实现票房折合为票房分,这个需要根据票房的实际数据确定函数，目前暂定一个线性函数
  function boxoffice2Score(uint BoxOffice)internal returns(uint){  //注意传入的票房单位为：千万
    uint boxOfficeScore=0;
    if(BoxOffice<=100){
  ...
  function calculateFilmScore(string memory filmId) internal{
    require(FilmEvaActivity[filmId].totalScore!=0&&FilmEvaActivity[filmId].totalForecastBoxOffice!=0, "totalScore err");
    
    //计算平均分,需要实现浮点数据，数据类型后面具体解决
    uint averageScore= FilmEvaActivity[filmId].totalScore/FilmEvaActivity[filmId].participateAgainNum;
    uint averageForecastBoxOffice=FilmEvaActivity[filmId].totalForecastBoxOffice/FilmEvaActivity[filmId].participateAgainNum;

    //计算最终影评分，当前采用线性函数进行关联两个因子
    uint boxOfficeScore=boxoffice2Score(averageForecastBoxOffice);
  ...
  function setFilmRealBoxOffice(string memory filmId,uint realBoxOffice)private{
    require(FilmEvaActivity[filmId].activityState==3, "this state can not set realboxoffice");
  ...
  //下述权重化为整型，解决浮点问题
  uint subWeight =4; //主观评分权重  
  uint objWeight =2; //客观票房预测权重
  uint subAndObjWeight =4; //主客观权重
  uint AvFactory=500; //放大因子
  //实现奖励算法
  function calculateRankScore(string memory filmId)private {

    uint scoreCoefficient;
    uint boxOfficeCoefficient;
    uint subAndObjCoefficient;
    uint averageScore= FilmEvaActivity[filmId].totalScore/FilmEvaActivity[filmId].participateAgainNum;
    uint averageForecastBoxOffice=FilmEvaActivity[filmId].totalForecastBoxOffice/FilmEvaActivity[filmId].participateAgainNum;
    uint realFilmBoxOfficeScore=boxoffice2Score(FilmEvaActivity[filmId].realFilmBoxOffice); //把实际票房转换为票房分
    ...
    for(uint i=0;i<FilmEvaActivity[filmId].participantAddress.length; i++){
      ...
      if((participantData.evaScore!=0)&&(participantData.forecastBoxOffice!=0)){
        ...
        uint forecastBoxOfficeScore=boxoffice2Score(participantBoxOffice);
        uint averageScoreBoxOffice=(participantScore+forecastBoxOfficeScore)/2;
